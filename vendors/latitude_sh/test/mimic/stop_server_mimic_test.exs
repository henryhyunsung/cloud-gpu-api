defmodule StopServerMimicTest do
  use ExUnit.Case
  use Mimic
  ...
  setup_all do
    success = %Tesla.Env{
      ...
      headers: [
        ...
        {"date", "Sat, 02 Sep 2023 10:40:38 GMT"},
        {"etag", "W/\"1ea7e41b4ecdd33ebd9a9857585a9bf9\""},
        {"server", "cloudflare"},
        {"vary", "Origin"},
        {"content-length", "101"},
        ...
        {"x-request-id", "4db2db11-4804-4b5d-8c1d-f24f20b174d3"},
        ...
        {"report-to",
         "{\"endpoints\":[{\"url\":\"https:\\/\\/a.nel.cloudflare.com\\/report\\/v3?s=FjR9ka7C2wVovOlMmENdoohYZIQovdTbwJgem94ickWzsiH6Pll2XT8HOYSao414j1HV8b0pbAuqOIqi%2FXLE7Cc%2BuDTd3ewMUkqKWGayTjy3Pvy764iudWOu2yhdbkMgpw%3D%3D\"}],\"group\":\"cf-nel\",\"max_age\":604800}"},
        {"nel",
         "{\"success_fraction\":0,\"report_to\":\"cf-nel\",\"max_age\":604800}"},
        {"cf-ray", "800522be189ea889-SYD"}
      ],
      body: "{\"data\":{\"id\":\"1693651238\",\"type\":\"actions\",\"attributes\":{\"status\":\"Powering device off\"}},\"meta\":{}}",
      status: 201,
      opts: [],
    ...
    list_stopped_server_success = %Tesla.Env{
      method: :get,
...

    {
      :ok,
      success: success,
      list_stopped_server_success: list_stopped_server_success,
  ...
  test "Testing successful stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:success]} end)
      |> Mimic.expect(:get_wrapper, 1, fn _url -> {:ok, context[:list_stopped_server_success]} end)
    assert {:ok, %{message: "Stopped server 18912", request_id: "1693651238"}} == LatitudeSh.stop_server(18_912)
  end

  test "Testing 400 error for stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:bad_request_error]} end)
    assert {:error, "Bad Request [400] -- General client error, possible malformed data."} == LatitudeSh.stop_server(18_912)
  end

  test "Testing 401 error for stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:unauthorized_error]} end)
    assert {:error, "Unauthorized [401] -- The API Key was not authorised (or no API Key was found)."} == LatitudeSh.stop_server(18_912)
  end

  test "Testing 403 error for stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:forbidden_error]} end)
    assert {:error, "Forbidden [403] -- The request is not allowed."} == LatitudeSh.stop_server(18_912)
  end

  test "Testing 404 error for stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:not_found_error]} end)
    assert {:error, "Not Found [404] -- The resource was not found."} == LatitudeSh.stop_server(18_912)
  end

  test "Testing 422 error for stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:unprocessable_entity_error]} end)
    assert {:error, "Unprocessable Entity [422] -- The data was well-formed but invalid."} == LatitudeSh.stop_server(18_912)
  end

  test "Testing 429 error for stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:too_many_requests_error]} end)
    assert {:error, "Too Many Requests [429] -- The client has reached or exceeded a rate limit, or the server is overloaded."} == LatitudeSh.stop_server(18_912)
  end

  test "Testing 500 error for stop_server call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:post_wrapper, 1, fn _url, _request_body -> {:ok, context[:server_error_error]} end)
    assert {:error, "Server Error [500] - Something went wrong on our end."} == LatitudeSh.stop_server(18_912)
  end
end
