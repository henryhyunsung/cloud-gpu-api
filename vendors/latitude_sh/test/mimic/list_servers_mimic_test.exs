defmodule ListServersMimicTest do
  use ExUnit.Case
  use Mimic
  ...
  setup_all do
    success = %Tesla.Env{
      ...
      headers: [
        ...
        {"date", "Sat, 02 Sep 2023 10:44:29 GMT"},
        {"etag", "W/\"447f0c55dcd48f5caeb16465c4444195\""},
        {"server", "cloudflare"},
        {"vary", "Origin"},
        {"content-length", "1468"},
        ...
        {"x-request-id", "868d5e1b-8f39-47af-a83b-b219754e6af6"},
        ...
        {"report-to",
         "{\"endpoints\":[{\"url\":\"https:\\/\\/a.nel.cloudflare.com\\/report\\/v3?s=wr2WGdyFeCzGTCeNgkTUdcF%2B8vwehgXrdgxN5xrhsZywN9oNhn8bCyKTwzDRT4FSp3ScRBZYUuO9vLlAJGUNODXGq46y1BXj250VG2UmqzPG8NRIV6crKAMjnXEaS3z1bQ%3D%3D\"}],\"group\":\"cf-nel\",\"max_age\":604800}"},
        {"nel",
         "{\"success_fraction\":0,\"report_to\":\"cf-nel\",\"max_age\":604800}"},
        {"cf-ray", "8005286f2c7fa889-SYD"}
      ],
  ...
  test "Testing successful list_servers call to mock api", context do
    expected_response = {:ok,
    [
      %{
        name: "c2-small-x86-sao-test02092023",
        status: :stopped,
        specs: %{
          os_type: "ubuntu",
          machine_type: "Bare Metal",
          ip_address: "177.54.156.71",
          accelerators: nil
        },
        plan: %{id: "20", slug: "c2-small-x86"},
        instance_id: "18912",
        region: %{country: "Brazil", site: "SAO"},
        billing_type: :on_demand
      }
    ]}
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:success]} end)
    assert expected_response == LatitudeSh.list_servers
  end

  test "Testing 400 error for list_servers call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:bad_request_error]} end)
    assert {:error, "Bad Request [400] -- General client error, possible malformed data."} == LatitudeSh.list_servers
  end

  test "Testing 401 error for list_servers call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:unauthorized_error]} end)
    assert {:error, "Unauthorized [401] -- The API Key was not authorised (or no API Key was found)."} == LatitudeSh.list_servers
  end

  test "Testing 403 error for list_servers call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:forbidden_error]} end)
    assert {:error, "Forbidden [403] -- The request is not allowed."} == LatitudeSh.list_servers
  end

  test "Testing 404 error for list_servers call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:not_found_error]} end)
    assert {:error, "Not Found [404] -- The resource was not found."} == LatitudeSh.list_servers
  end

  test "Testing 422 error for list_servers call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:unprocessable_entity_error]} end)
    assert {:error, "Unprocessable Entity [422] -- The data was well-formed but invalid."} == LatitudeSh.list_servers
  end

  test "Testing 429 error for list_servers call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:too_many_requests_error]} end)
    assert {:error, "Too Many Requests [429] -- The client has reached or exceeded a rate limit, or the server is overloaded."} == LatitudeSh.list_servers
  end

  test "Testing 500 error for list_servers call to mock api", context do
    LatitudeSh.ApiWrappers
      |> Mimic.expect(:get_wrapper, 1, fn _url, _query_params -> {:ok, context[:server_error_error]} end)
    assert {:error, "Server Error [500] - Something went wrong on our end."} == LatitudeSh.list_servers
  end
